...
def _sf_query(sql: str) -> pd.DataFrame:
    """Run a Snowflake query safely. Returns empty df on any error."""
    try:
...
with st.sidebar:
    ...
    d_start = pd.Timestamp(dr[0]) if isinstance(dr,(list,tuple)) else pd.Timestamp(dr)
    d_end   = pd.Timestamp(dr[1]) if isinstance(dr,(list,tuple)) and len(dr)>1 else d_start
